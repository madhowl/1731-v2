...
class CalculatorApp:
    ...
    def create_buttons(self):
        ...
        equal_btn = tk.Button(
            ...
            font=("Arial", 14),
            bg="#4CAF50",
            command=self.equal_click
        )
        equal_btn.grid(row=4, column=3, sticky="nsew", padx=2, pady=2)
    ...
    def operator_click(self, operator):
        ...
        self.previous = self.display_var.get()
    ...
    def equal_click(self):
        ...
        try:
            current = self.display_var.get()
            expression = f"{self.previous}{self.operator}{current}"
...
def main():
    ...
    print("Решения для практического занятия 17: Создание GUI с помощью Tkinter")
    print("=" * 70)
    print("Выберите приложение для запуска:")
    print("1 - Базовое окно")
    print("2 - Коллекция виджетов")
    print("3 - Менеджеры размещения")
    ...
    if choice == "1":
        ...
        app = WidgetCollectionApp()
        app.run()
    elif choice == "3":
        app = LayoutManagementApp()
        app.run()
    elif choice == "4":
        app = EventHandlingApp()
        app.run()
    elif choice == "5":
        app = CalculatorApp()
        app.run()
