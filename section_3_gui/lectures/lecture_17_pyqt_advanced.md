# Лекция 17: PyQt6 - продвинутый уровень

## Продвинутые виджеты, MVC/MVP паттерны, многопоточность

### План лекции:
1. Продвинутые виджеты PyQt6
2. Архитектурные паттерны в GUI-приложениях
3. Многопоточность в PyQt6
4. Пользовательские виджеты
5. Работа с файловой системой
6. Qt Designer для продвинутых интерфейсов
...
class AdvancedWidgetsDemo(QMainWindow):
    """
    Демонстрация продвинутых виджетов PyQt6
    """
    def __init__(self):
        super().__init__()
        self.init_ui()
    ...
    def init_ui(self):
        self.setWindowTitle('Продвинутые виджеты PyQt6')
    ...
    def create_list_tab(self):
        ...
        for i in range(5, 8):
            ...
            item.setCheckState(Qt.CheckState.Checked if i % 2 == 0 else Qt.CheckState.Unchecked)
...
class ThreadingDemo(QWidget):
    ...
    def init_ui(self):
        ...
        # Заголовок
        title = QLabel('Многопоточность в PyQt6')
        title.setAlignment(Qt.AlignmentFlag.AlignCenter)
        title.setStyleSheet("font-size: 16px; font-weight: bold; margin: 10px;")
        layout.addWidget(title)
...
def run_custom_widgets_demo():
    ...
    main_window.setWindowTitle('Пользовательские виджеты PyQt6')
...
```python
from PyQt6.QtWidgets import QFileSystemModel, QTreeView, QListView, QSplitter
from PyQt6.QtCore import QDir
from PyQt6.QtGui import QFileInfo
import os
...
class FileSystemBrowser(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.file_model.setFilter(QDir.Filter.AllEntries | QDir.Filter.NoDotAndDotDot | QDir.Filter.AllDirs)
...
### Создание сложных форм в Qt Designer

Qt Designer позволяет создавать сложные интерфейсы визуально:

1. Создайте новую форму в Qt Designer
2. Добавьте QTabWidget для создания вкладок
3. Используйте QSplitter для разделения областей
4. Настройте сигналы и слоты визуально
5. Сохраните как .ui файл и загрузите в приложении

### Пример загрузки сложной формы:
...
class ComplexApp(QMainWindow):
    def __init__(self):
        super().__init__()
        uic.loadUi('complex_form.ui', self)
        
        # Подключение сигналов после загрузки UI
        self.tab_widget.currentChanged.connect(self.on_tab_changed)
        
        # Настройка дополнительной логики
        self.setup_custom_logic()
    
    def on_tab_changed(self, index):
        print(f"Переключено на вкладку {index}")
...
PyQt6 предоставляет мощный и гибкий инструментарий для создания графических интерфейсов в Python. Продвинутые виджеты, такие как QTableWidget, QTreeWidget и QListView, позволяют создавать сложные интерфейсы. Архитектурные паттерны MVC и MVP помогают организовать код приложения. Многопоточность позволяет выполнять длительные операции без блокировки интерфейса. Создание пользовательских виджетов расширяет возможности стандартных компонентов.

## Контрольные вопросы:
1. Какие продвинутые виджеты доступны в PyQt6?
2. В чем отличие паттернов MVC и MVP?
3. Как реализовать многопоточность в PyQt6?
4. Как создать пользовательский виджет?
5. Как работать с файловой системой в PyQt6?
6. Как использовать Qt Designer для создания сложных интерфейсов?
