...
# Решение задания 1: Обработка кликов мыши
class MouseEventSolution:
    """Решение для задания 1: Обработка кликов мыши"""
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("Решение задания 1: Обработка кликов мыши")
...
# Решение задания 2: Обработка клавиатуры
class KeyboardEventSolution:
    """Решение для задания 2: Обработка клавиатуры"""
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("Решение задания 2: Обработка клавиатуры")
        self.root.geometry("700x600")
...
# Решение задания 3: Передача данных через события
class DataTransferSolution:
    """Решение для задания 3: Передача данных через события"""
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("Решение задания 3: Передача данных через события")
        self.root.geometry("700x600")
...
# Решение задания 4: Кастомные события
class CustomEventSolution:
    """Решение для задания 4: Кастомные события"""
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("Решение задания 4: Кастомные события")
        self.root.geometry("700x600")
    ...
    def toggle_subscription(self):
        ...
        if self.subscribed:
            ...
            self.event_label.config(text="Подписка активна")
        else:
...
# Решение задания 5: Комплексное приложение "Рисовалка"
class DrawingAppSolution:
    """Решение для задания 5: Комплексное приложение - Рисовалка"""
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("Решение задания 5: Рисовалка")
...
def demonstrate_event_handling_patterns():
    """Демонстрирует различные паттерны обработки событий"""
    print("=== Демонстрация паттернов обработки событий ===")
    
    # Mouse events
    print("\n1. События мыши:")
    print("   - Button-1: Левый клик")
    print("   - Button-3: Правый клик")
    ...
    print("   - KeyRelease: Отпускание клавиши")
    print("   - Control+X: Комбинации клавиш")
    
    # Custom events
    print("\n3. Кастомные события:")
    print("   - <<CustomEventName>>: Собственные события")
    print("   - event_generate(): Генерация событий")
    print("   - bind(): Привязка обработчиков")
    
    # Data transfer through events
    print("\n4. Передача данных через события:")
    print("   - Lambda-функции для передачи параметров")
    print("   - Свойства event.widget для доступа к виджету")
    print("   - Атрибуты объекта события")

def compare_event_handling_implementations():
    """Сравнение различных реализаций обработки событий"""
    print("\n=== Сравнение реализаций обработки событий ===")
    print("""
    1. Метод bind():
       + Гибкость в привязке к конкретным виджетам
       + Возможность обработки сложных комбинаций
       + Четкое разделение обработчиков
       - Может привести к "спагетти"-коду при большом количестве привязок
       - Требует хорошего понимания объекта события
    
    2. Команды (command):
       + Простота использования
       + Автоматическая передача контекста
       - Ограниченная гибкость (только для виджетов с поддержкой command
       - Невозможность получения детальной информации о событии
    
    3. Кастомные события:
       + Возможность связи между разными компонентами
       + Повышенная гибкость архитектуры
       - Сложнее в отладке
       - Требует четкого понимания потока событий
    
    4. Lambda-функции:
       + Простота передачи параметров
       + Возможность динамической генерации обработчиков
       - Может усложнить отладку
       - Неэффективно при большом количестве обработчиков
    """)

# Примеры использования:
if __name__ == "__main__":
    print("=== Решения для практического занятия 19 ===")
    
    print("\n1. Решение задания 1: Обработка кликов мыши")
    mouse_solution = MouseEventSolution()
    # mouse_solution.run()  # Закомментировано для предотвращения автоматического запуска
    
    print("\n2. Решение задания 2: Обработка клавиатуры")
    keyboard_solution = KeyboardEventSolution()
    # keyboard_solution.run()  # Закомментировано
    
    print("\n3. Решение задания 3: Передача данных через события")
    data_solution = DataTransferSolution()
    # data_solution.run()  # Закомментировано
    
    print("\n4. Решение задания 4: Кастомные события")
    custom_solution = CustomEventSolution()
    # custom_solution.run()  # Закомментировано
    
    print("\n5. Решение задания 5: Комплексное приложение (Рисовалка)")
    drawing_solution = DrawingAppSolution()
    # drawing_solution.run()  # Закомментировано
    
    print("\n6. Дополнительные примеры")
    advanced_solution = AdvancedEventExamples()
    # advanced_solution.run()  # Закомментировано
    
    print("\n7. Дополнительная информация")
    demonstrate_event_handling_patterns()
    compare_event_handling_implementations()
