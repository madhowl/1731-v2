...
## Цель занятия
Научиться использовать абстрактные классы интерфейсы в Python для создания четкой архитектуры игровых систем и обеспечения обязательной реализации методов в дочерних классах игровых сущностей.

## Задачи

### Задача 1: Создание абстрактного класса игровой сущности (25 баллов)
Создайте абстрактный класс `GameEntity` с:
- Абстрактным методом `interact(other_entity)` для взаимодействия с другими сущностями
- Абстрактным методом `update(delta_time)` для обновления состояния
- Конкретным методом `get_info()` для получения информации о сущности
...
### Задача 2: Реализация интерфейса в игровых сущностях (25 баллов)
Создайте интерфейс `Attackable` с методом `take_damage(damage)` и `deal_damage(target)`, который:
- Реализуется в классах игровых персонажей и монстров
- Позволяет производить боевые действия
...
### Задача 3: Комбинирование абстрактных классов для игровых систем (25 баллов)
Создайте абстрактный класс `Character` с:
- Абстрактными методами `use_special_ability()` и `level_up()`
- Конкретным методом `rest()` для восстановления здоровья
- Реализуйте наследников: `Warrior`, `Mage`, `Archer`
...
### Задача 4: Применение ABC в реальном игровом примере (25 баллов)
Реализуйте систему управления инвентарем:
- Абстрактный класс `InventoryItem` с методом `use(character)`
- Конкретные классы: `HealthPotion`, `ManaPotion`, `WeaponItem`
- Обработку ошибок при неправильном использовании предметов
...
3. Не забывайте про наследование и реализацию абстрактных методов в игровых сущностях
4. Используйте аннотации типов для лучшей читаемости кода
5. В абстрактных классах можно комбинировать абстрактные и конкретные методы

## Требования к отчету
- Исходный код программ с игровыми сущностями
- Объяснение, почему выбрана та или иная архитектура для игровых систем
- Примеры использования абстрактных классов в игровом контексте

## Критерии оценки
- Правильное использование абстрактных классов: 40%
- Корректная реализация наследников игровых сущностей: 30%
- Качество кода и документация в игровом контексте: 30%

## Практические задания

### Задание 1: Реализация абстрактного класса GameEntity

Создайте конкретные классы, наследующиеся от `GameEntity`:
- `PlayerCharacter` - игрок
- `NonPlayerCharacter` - NPC
- `GameItem` - игровой предмет

Каждый класс должен реализовать абстрактные методы по-своему в соответствии с игровой логикой.
...
### Задание 2: Реализация интерфейса Attackable

Создайте классы, реализующие интерфейс `Attackable`:
- `Warrior` - воин
- `Orc` - орк
- `Dragon` - дракон

Каждый класс должен реализовать методы `take_damage` и `deal_damage` в соответствии с его характеристиками.
...
### Задание 3: Реализация абстрактного класса Character

Создайте конкретные классы персонажей, наследующиеся от `Character`:
- `Knight` - рыцарь
- `Wizard` - волшебник
- `Ranger` - рейнджер

Каждый класс должен реализовать абстрактные методы и использовать метод `rest`.
...
### Задание 4: Реализация системы инвентаря

Создайте конкретные классы предметов, наследующиеся от `InventoryItem`:
- `HealthPotion` - зелье здоровья
- `ManaPotion` - зелье маны
- `Sword` - меч

Каждый класс должен реализовать метод `use` по-своему в соответствии с типом предмета.
